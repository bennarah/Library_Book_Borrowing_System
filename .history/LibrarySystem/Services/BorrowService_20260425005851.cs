...
namespace LibrarySystem.Services
{
    public class BorrowService : IBorrowService
    {
        ...
        {
            // Load as tracked entity so EF will include the RowVersion in the UPDATE WHERE clause
            ...
            // Decrement while the RowVersion is still tracked — EF will append
            // "WHERE RowVersion = <original>" to the UPDATE, so a concurrent
            // borrow that already decremented will cause SaveChanges to throw.
            ...
            try
            {
                // CreateAsync calls SaveChangesAsync, which persists both the
                // BorrowRecord insert and the dirty Book update in one round-trip.
        ...
        public async Task<BorrowResponseDto?> ReturnAsync(int id)
        {
            ...
            // Load book as tracked entity so the increment is saved alongside the record update.
            ...
            try
            {
                // UpdateAsync calls SaveChangesAsync — persists both the record
                // update and the book increment in one round-trip.
